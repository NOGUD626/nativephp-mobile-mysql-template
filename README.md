...
先に書いておくと、NativePHP の最大の魅力 — **Laravel スキルそのままモバイル化できる /
Swift / Kotlin を書かなくていい / Composer / Artisan の普段のワークフローで端末アプリを
書ける** — このバリューは API 経由構成でもまるごと残る。Sanctum で API を建てて
NativePHP を被せる構成は公式が一押ししている王道で、ここに異論はない。

一方で、Introduction が掲げる **"No web server required"** と **"offline-first"** という
看板は、API バックエンドを足した瞬間に文字通りの意味では成立しなくなる。
「サーバを足さずに端末配布だけで完結させたい」を重視するケースだけは、推奨ルートと
旨みが微妙にずれる、という構図がある。

| 配布形態 | DB アクセス | スローガンとの距離 | 推奨度 |
|---|---|---|---|
| App Store / Google Play で広く配布 | API 経由 (公式推奨) | offline-first は前提条件付き | 公式 ✅ |
| クラウド業務 (社内 SaaS 的) | API 経由 (公式推奨) | offline-first は前提条件付き / 運用コストは許容 | 公式 ✅ |
| オンプレ業務 / 社内 LAN 閉域 | API 経由 | 前提条件付き + 現地 API 運用コスト | 採用ハードル高 |
| **オンプレ業務 / 社内 LAN 閉域** | **MySQL 直結 (本テンプレ)** | **スローガンとの一致が完全 — 端末配布だけで完結** | 公式 🚫 / 現場 ✅ |
